...
func TestCompare(t *testing.T) {
	...
	if maxHand := analyzeHandStr("Qs9dAh6hAc6d9c").getMaxHands(); maxHand.MaxCase != TwoPair {
	...
	if maxHand := analyzeHandStr("Jd5d9cXnKs2c6d").getMaxHands(); maxHand.MaxCase != OnePair {
		t.Errorf("PokerType 错误，应该为 %d，结果为 %d", OnePair, maxHand.MaxCase)
	} else if maxHand.MaxHand != 16779920 {
		t.Errorf("Value 错误，应该为\n%b，结果为 \n%b", 16779920, maxHand.MaxHand)
	}

	if maxHand := analyzeHandStr("9cXnKs2c6dTc7c").getMaxHands(); maxHand.MaxCase != Flush {
		t.Errorf("PokerType 错误，应该为 %d，结果为 %d", Flush, maxHand.MaxCase)
	} else if maxHand.MaxHand != 4513 {
		t.Errorf("Value 错误，应该为\n%b，结果为 \n%b", 4513, maxHand.MaxHand)
	}

	if maxHand := analyzeHandStr("Ad3hQhJc2c8s2s").getMaxHands(); maxHand.MaxCase != OnePair { // 一对 11011000000001
		t.Errorf("PokerType 错误，应该为 %d，结果为 %d", OnePair, maxHand.MaxCase)
	...
	if maxHand := analyzeHandStr("6dXnAc3c7dAh7s").getMaxHands(); maxHand.MaxCase != FullHouse {
	...
	if maxHand := analyzeHandStr("JdJcJsJh3d4d6d").getMaxHands(); maxHand.MaxCase != FourOfAKind {
	...
	// 一对
	if maxHand := analyzeHandStr("JdJc5hKh3d4d6d").getMaxHands(); maxHand.MaxCase != OnePair {
		t.Errorf("PokerType 错误，应该为 %d，结果为 %d", OnePair, maxHand.MaxCase)
	...
	// 同花顺 111110
	if maxHand := analyzeHandStr("XnJc5dKh3d4d6d").getMaxHands(); maxHand.MaxCase != StraightFlush {
	...
	// 同花 1000011110
	if maxHand := analyzeHandStr("JdJc5dKh3d4d6d").getMaxHands(); maxHand.MaxCase != Flush {
		t.Errorf("PokerType 错误，应该为 %d，结果为 %d", Flush, maxHand.MaxCase)
	...
	if maxHand := analyzeHandStr("JdJcKcKh3d4d6d").getMaxHands(); maxHand.MaxCase != TwoPair {
		t.Errorf("PokerType 错误，应该为 %d，结果为 %d", TwoPair, maxHand.MaxCase)
